...
from laytime_agent import extract_laytime_from_docs, extract_excel_data_from_docs
...
if st.button("Extract and Analyze") and uploaded_files:
    ...
    # Final block: generate Excel if both Contract and SoF were extracted
        if "Contract" in extracted_data and "SoF" in extracted_data:
            ...
            # ✅ Flatten metadata without overwriting important structure
            contract = {
                **contract_raw,
                **contract_raw.get("metadata", {}),
                "Sections": contract_raw.get("Sections", [])
            }

            sof = {
                **sof_raw,
                **sof_raw.get("metadata", {})
            }

            # ✅ Extract to Excel DataFrame
            laytime_agent_response, raw_response = extract_laytime_from_docs(contract_raw, sof_raw)
            excel_df = extract_excel_data_from_docs(laytime_agent_response)

            st.subheader("✅ Populating Excel with extracted fields")
            ...
            excel_filename = f"Laytime_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"

            # ✅ Save Excel to temp and enable download
            with tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx") as tmp_xlsx:
                excel_df.to_excel(tmp_xlsx.name, index=False)
                tmp_xlsx.seek(0)
                with open(tmp_xlsx.name, "rb") as f:
                    st.download_button(
                        label="📥 Download Laytime Report",
                        data=f.read(),
