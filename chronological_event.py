    ...
    prompt = f"""
        ...
        Given the following chronological list of time blocks (events), your task is to identify and fill any temporal gaps between consecutive events. For each gap identified, you must insert a new event block with an inferred reason, from the "block['Reason']" of corresponding date and time.
            ...
            2.  **Identify 
                ...
                *If end_time of last event = none then end_time of that event = start_time of that event.
                *Ensure that data is grouped by date, and for every new date, a new block of rows should begin, the last event of that particular date should have the end_time as 23:59 except for the last event.
